...
function createPowerFlowObjects(adapter, obj) {
    for (var id in obj.Inverters){
        ...
        addObjectNotExists(adapter,"inverter." + id,"E_Day","Produced energy current day","number","Wh",obj.Inverters[id],"","value");
        addObjectNotExists(adapter,"inverter." + id,"E_Year","Produced energy current year","number","Wh",obj.Inverters[id],"","value");
        addObjectNotExists(adapter,"inverter." + id,"E_Total","Produced energy total","number","Wh",obj.Inverters[id],"","value");
        addObjectNotExists(adapter,"inverter." + id,"P","Actual PV AC power","number","W",obj.Inverters[id],"","value");
        addObjectNotExists(adapter,"inverter." + id,"SOC","Relative charging state of battery","number","%",obj.Inverters[id],"","value");
    }
    ...
    setTimeout(function(data){
        createStates(adapter,data.Site, "site");
        createStates(adapter,data.Inverters, "inverter");
    },1000,obj);
...
function createInverterInfoObjects(adapter, obj) {
    ...
    for (var id in obj){
        ...
        addObjectNotExists(adapter,"inverter." + id,"PVPower","PV peak power","number","W",obj[id],"","value");
    }
