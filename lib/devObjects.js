...
function createInverterObjects(adapter, id, obj) {
    try {
        ...
        addObjectNotExists(
            ...
            `inverter.${id}`,
            'YEAR_ENERGY',
            'Produced energy in current year',
            'number',
            'Wh',
            obj,
            '',
        ...
        addObjectNotExists(
            adapter,
            `inverter.${id}`,
            'DeviceStatus.InverterErrorState',
            'Inverter Error state',
            'string',
            '',
            null,
        );
...
function createArchiveObjects(adapter, id, obj) {
    try {
        ...
        addObjectNotExists(
            ...
            'Temperature of Inverter Powerstage',
            'number',
            '°C',
            obj,
            '',
        ...
        if (
            Object.prototype.hasOwnProperty.call(obj, 'Voltage_DC_String_1') &&
            Object.prototype.hasOwnProperty.call(obj, 'Current_DC_String_1')
        ) {
            addObjectNotExists(
                adapter,
                `inverter.${id}`,
                'Power_DC_String_1',
                'DC Power of string 1',
                'number',
                'W',
                null,
                '',
        ...
        if (
            Object.prototype.hasOwnProperty.call(obj, 'Voltage_DC_String_2') &&
            Object.prototype.hasOwnProperty.call(obj, 'Current_DC_String_2')
        ) {
            addObjectNotExists(
                adapter,
                `inverter.${id}`,
                'Power_DC_String_2',
                'DC Power of string 2',
                'number',
                'W',
                null,
                '',
...
function createStorageObjects(adapter, id, obj) {
    try {
        ...
        addObjectNotExists(
            ...
            'Relative charging state of battery',
            'number',
            '%',
            obj.Controller,
            '',
...
function createOhmPilotObjects(adapter, id, obj) {
    try {
        ...
        setTimeout(
            function (obj) {
                createStates(adapter, obj, `ohmpilot.${id}`);
            },
            1000,
            obj,
        );
    ...
    try {
        ...
        for (const key in apiObject) {
            if (apiObject[key.toString()] === null) {
                adapter.log.silly(`API Objekt ${key.toString()} is null, no object created!`);
    ...
    try {
        ...
        if (apiObject === null || apiObject == 'undefined') {
            ...
            if (apiObject[stateId] === null) {
                adapter.log.silly(`Property ${stateId} is null. No object created!`);
                return;
            } else if (apiObject[stateId] != null && typeof apiObject[stateId] == 'object') {
                if (
                    Object.prototype.hasOwnProperty.call(apiObject[stateId], 'Value') &&
                    apiObject[stateId]['Value'] === null
                ) {
                    adapter.log.silly(`Property ${stateId} is null. No object created!`);
                    return;
                } else if (
                    Object.prototype.hasOwnProperty.call(apiObject[stateId], 'Value') &&
                    typeof apiObject[stateId]['Value'] != 'object'
                ) {
            ...
            adapter.log.silly(
                `Object ${JSON.stringify(apiObject)} does not have a property ${
                    stateId
                }, therefore no State was created`,
            );
        }
    } catch (ex) {
